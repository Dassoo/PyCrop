    ...
    for directory, subdirectories, files in os.walk(inputPath):
        for file in files:
            if(file.endswith('.CR2')):
                ...
                rgb_im.save(outputPath+'\\temp\\'+file[:size-4]+'.jpg')
...
def cropping(inputPath, outputPath, l_padding, t_padding, r_padding, b_padding, imgQuality, application, imgViewer, outputFilename, whiteBalance):
    ...
    for directory, subdirectories, files in os.walk(outputPath+'\\temp'):
        for file in files:
            path = outputPath+'\\temp\\'+file
            ...
            contours, _= cv2.findContours(th_er1, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
...
def cpApplication(inputPath, outputPath, cpPath, exportFormat):
    if(not cpPath == False):
        ...
        os.system('start rawtherapee-cli.exe -o '+outputPath+'\\jpg_export -p '+cpPath+' -Y -c '+outputPath+'\\cropped')
...
def main():
    ...
    try:      
        ...
        if not os.path.exists(outputPath+'\\temp'): 
            os.makedirs(outputPath+'\\temp',exist_ok=True)
        if(args['Formato TIFF'] == True):
            if not os.path.exists(outputPath+'\\tiff_export'): 
                os.makedirs(outputPath+'\\tiff_export',exist_ok=True)
